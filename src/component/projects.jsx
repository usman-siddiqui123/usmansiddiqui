...
const projectsData = [
  ...
  },
  {
    ...
    title: "Website UI",
    description:
      "The User Interface (UI) of a website is the part that users see and interact with directly. A good UI design makes a website visually appealing and enhances the overall user experience. The right balance of colors, typography, buttons, and layout creates an engaging and well-organized interface. When the UI is clean and intuitive, visitors tend to stay longer and explore more content with ease.",
  },
  {
    id: 5,
    image: temp5,
...
export default function Projects() {
  return (
    ...
    >
      ...
      {/* Title Section */}
      <div className="relative z-10 flex flex-col items-center justify-center text-center mt-10 px-4 sm:px-6">
      ...
      {/* 🔹 Projects Grid */}
      <div className="z-10 mt-12 px-4 sm:px-8 md:px-12 lg:px-20 flex flex-col items-center">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-10 mb-12 text-center w-full max-w-7xl">
          {projectsData.map((project) => (
            <div
              key={project.id}
              className="bg-white/10 backdrop-blur-md rounded-2xl p-4 sm:p-6 shadow-xl border border-purple-500 hover:scale-105 transition-all duration-300 flex flex-col justify-between"
            >
              <img
                src={project.image}
                alt={project.title}
              ...
              />
              <h3 className="text-lg sm:text-xl font-semibold mb-2 text-teal-400">
                {project.title}
              </h3>
